...
document.addEventListener('DOMContentLoaded', ars_init_todo_list)
function ars_init_todo_list() {
  ...
  let formElements = {
    ...
    formEl        : { el: ars_create_new_element('form','', [ ars_get_attr_obj('id', 'form_todolist'), ars_get_attr_obj('name', 'form_todolist') ] ), prevParent:'parentEl' },
    formDiv       : { el: ars_create_new_element('div','', [ ars_get_attr_obj('class', 'col-auto') ] ), prevParent:'formEl' },
    formLabel     : { el: ars_create_new_element('label', 'What to do?', [ ars_get_attr_obj('class', 'sr-only'), ars_get_attr_obj('for', 'todo_text') ] ), prevParent:'formDiv' },
    ...
    fieldInput    : { el: ars_create_new_element('input','', [ ars_get_attr_obj('type', 'text'), ars_get_attr_obj('class', 'form-control'), ars_get_attr_obj('name', 'todo_text'), ars_get_attr_obj('id', 'todo_text'), ars_get_attr_obj('placeholder', 'What to do?')  ] ), prevParent:'inputTextDiv' },
    formButton    : { el: ars_create_new_element('button', 'ADD', [ ars_get_attr_obj('type', 'submit'), ars_get_attr_obj('class', 'btn btn-primary w-100') ] ), prevParent:'formDiv' },
  }
  let listElements = {
  ...
  todoListForm = document.getElementById('form_todolist')
  ars_render_todo_list(pageEl, listElements)

  // Insert list item
  todoListForm.addEventListener('submit', (e)=>{
    e.preventDefault()
    let todoInput = todoListForm.elements['todo_text']
    if( todoInput.value ){
      ars_render_list_item(pageEl.querySelector('ul.list-group'), todoInput.value)
      todoInput.value=''
      todoInput.focus()
    }
  })
  ...
  document.addEventListener('click', (e, i)=>{
    if( e.target && e.target.classList.contains('action-event') ){
      e.preventDefault()
